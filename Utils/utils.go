...
func EraseInit() bool {
	...
	msg2, _ := hex.DecodeString("029ff8000000000000000001000007")

	// ################1

	// Assuming you have a "send" and "receive" function for communication

	data1 := binary.BigEndian.Uint64(msg1)
	data2 := binary.BigEndian.Uint64(msg2)
	...
	send(0x9ff8, binary.BigEndian.Uint64(msg1))
	time.Sleep(30 * time.Millisecond)
	result, _ = receive(0x9ff0, 24, 5, true, 0, 0x01, false)
	fmt.Printf("%x\n", result)
	...
	// Assuming you have a "send" and "receive" function for communication
	send(0xa000, binary.BigEndian.Uint64(msg1))
	// time.Sleep(30 * time.Millisecond)
	send(0x9ff8, binary.BigEndian.Uint64(msg2))
	// time.Sleep(3 * time.Millisecond)
	result, _ = receive(0x9ff0, 24, 5, true, 0, 0x01, false)
	fmt.Printf("%x\n", result)
