...
const (
	// Replace these with the actual VID and PID of your USB device.
	targetVID gousb.ID = 0x248a
	targetPID gousb.ID = 0x5320
)

func send(wValue uint16, dataOrWLength uint64) error {
	...
	// Define the maximum allowed data size for the control transfer (adjust this value as needed).
	maxDataSize := 64 // For example, set the maximum allowed data size to 64 bytes.

	// Perform multiple control transfers if data size exceeds the maximum allowed.
	var sentData uint64
	for sentData < dataOrWLength {
		// Calculate the remaining data to be sent in this transfer.
		remainingData := dataOrWLength - sentData
		dataSize := uint64(maxDataSize)
		if remainingData < dataSize {
			dataSize = remainingData
		}

		// Prepare the control transfer parameters for this chunk of data.
...
func NewSend(wValue uint16, dataOrWLength uint64, msg []byte) error {
	ctx := gousb.NewContext()
	defer ctx.Close()
...
func receive(wValue uint16, dataOrWLength, timeOut int, reset bool, testIndex int, testData byte, testLast bool) ([]byte, error) {
	fmt.Println("Entering receive")
	...
	if time.Now().After(timeoutStart) && timeOut != 0 {
		fmt.Println("Exiting receive 4")
		return nil, fmt.Errorf("Timeout")
	}

	lastMessage = data

	fmt.Println("Exiting receive final")
	return data, nil
}
...
func EraseInit() bool {
	...
	// Assuming you have a "send" and "receive" function for communication

	// data1 := 0
	// data2 := 0

	NewSend(0x9fff, 0, msg1)
	time.Sleep(30 * time.Millisecond)
	NewSend(0x9ff8, 0, msg2)
	time.Sleep(3 * time.Millisecond)
	result, _ := receive(0x9ff0, 24, 5, true, 0, 0x01, false)
	fmt.Printf("%x\n1", result)
	...
	result, _ = receive(0xa000, 9, 5, true, 0, 0xab, false)
	fmt.Printf("%x\n", result)
	...
	result, _ = receive(0xa000, 9, 5, true, 0, 0x05, false)
	fmt.Printf("%x\n", result)
	...
	msg1, _ = hex.DecodeString(string1)
	...
	NewSend(0xa000, 0, msg1)
	NewSend(0x9ff8, 0, msg2)
	result, _ = receive(0x9ff0, 24, 5, true, 0, 0x04, false)
	fmt.Printf("%x\n", result)
...
func EraseAdr(adr int) bool {
	...
	time.Sleep(3 * time.Millisecond)
	NewSend(0x9ff8, 0, msg2)
	receive(0x9ff0, 24, 5, true, 0, 0x09, false)
	// fmt.Printf("%x", result)
	...
	time.Sleep(30 * time.Millisecond)
	NewSend(0x9ff8, 0, msg2)
	receive(0x9ff0, 24, 5, true, 0, 0x01, false)
	// fmt.Printf("%x\n", result)
	...
	for {
		...
		result, _ = receive(0xa000, 9, 0, false, 0, 0x01, false)

		if time.Now().After(timeoutStart) {
			return false
		}

		if len(result) > 0 && result[0] == 0x4d {
			break
		}
	}
	...
	NewSend(0xa000, 0, msg1)
	receive(0x9ff0, 24, 5, true, 0, 0x0c, false)

	receive(0xa000, 20, 0, false, 0, 0, false)
...
func Reset() bool {
	...
	err = NewSend(0xa000, 0, msg1)
	if err != nil {
		return false
	}
	err = NewSend(0x9ff8, 0, msg2)
	if err != nil {
		return false
	}
	receive(0x9ff0, 24, 5, true, 0, 0x01, false)
